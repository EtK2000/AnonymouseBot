...
Visualizer = function(container, options, w, h, configOverrides) {
	...
	try {
        ...
        */
		this.imgMgr.put('treasure_closed', 'coins.png');
		...
		this.imgMgr.put('treasure_on_ship', '4-Ships_coins.png');
		this.imgMgr.put('barrel', 'barrel_flip.png');
		this.imgMgr.put('ship_defend', 'ship-sprite-defense-n.png');
...
Visualizer.prototype.tryStart = function() {
	...
	if (this.state.replay) {
        ...
        this.imgMgr.patterns['treasureOnShip'] = this.imgMgr.images['treasure_on_ship'];
		...
		*/
		...
		this.map.treasureOnShip = this.imgMgr.patterns['treasureOnShip'];
...
Visualizer.prototype.calculateHint = function() {
	...
	this.state.replay.meta['replaydata']['treasures'].forEach(function(treasure) {
		...
		if (y === row && x === col && (treasure[2][turn-1] === '1' || turn === 0)) {
