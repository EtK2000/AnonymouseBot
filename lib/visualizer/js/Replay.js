	...
	if (this.turns[n] === undefined) {
		...
		for (i = 0; i < pirates.length; i++) {
			...
			if (pirate[2] === n + 1 || n === 0 && pirate[2] === 0) {
				// spawn this pirate
				if (this.revision >= 3) {
					aniAnt = this.spawnPirate(i, pirate[0], pirate[1], pirate[2], pirate[4], pirate[5], pirate[7], pirate[8], pirate[9], pirate[10], pirate[11], pirate[12], pirate[13]);
				} else {
...
Replay.prototype.spawnPirate = function(id, row, col, spawn, owner, moves, gameId, reasonOfDeath, treasureHistory, attackHistory, defenseHistory, drinkHistory, attackRadius) {
	var aniAnt = this.aniAnts[id] = new Pirate(id, gameId, spawn - 0.25, reasonOfDeath, treasureHistory, attackHistory, defenseHistory, drinkHistory, attackRadius);
